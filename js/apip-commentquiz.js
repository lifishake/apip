document.addEventListener('DOMContentLoaded', function(){
  ...
  var buildQuiz = function(quizNode){
    ...
    var errorText = quizNode.getAttribute('data-apipcommentquiz-error');
    var questions = parseQuiz(quizNode.getAttribute('data-apipcommentquiz'));
    var correctId = 'apipcommentquiz' + location.pathname + questions.map(function(q){return q.correct}).join('');
    ...
    questions.forEach(function(question, index){
      ...
      question.answer
        .map(function(key, val){return key && buildAnswer(key, 'apipcommentquiz' + index, val)})
    ...
    quizNode.addEventListener('change', function(){
      var checked = questions.map(function(q,i){return container.querySelector('input[name="' + 'apipcommentquiz' + i + '"]:checked')});
  ...
  [].forEach.call(document.querySelectorAll('.' + 'apipcommentquiz'), buildQuiz);
});